...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, seg_num_classes=2, num_classes=1000, embed_dim=768, depth=12,
        ...
        #nn.init.kaiming_normal_(self.concate_conv.weight, mode='fan_out', nonlinearity='relu')
        
        #resent part
        resnet = models.resnet34(pretrained=True)
        filters=[64,64,128,256,512]
        self.firstconv_1 = nn.Conv2d(in_channels=1, out_channels=64, kernel_size=7, stride=2, padding=3, bias=False)
        self.firstbn_1 = resnet.bn1
        self.firstrelu_1 = resnet.relu
        self.firstmaxpool_1 = resnet.maxpool
        self.encoder1_1 = resnet.layer1
        #self.encoder2_1 = resnet.layer2
        #self.encoder3_1 = resnet.layer3

        self.firstconv_2 = nn.Conv2d(in_channels=1, out_channels=64, kernel_size=7, stride=2, padding=3, bias=False)
        self.firstbn_2 = resnet.bn1
        self.firstrelu_2 = resnet.relu
        self.firstmaxpool_2 = resnet.maxpool
        self.encoder1_2 = resnet.layer1
        #self.encoder2_2 = resnet.layer2
        #self.encoder3_2 = resnet.layer3

        #self.decode1 = DecoderBlock(in_channels=filters[3]*2, mid_channels=filters[3]*4, out_channels=filters[3], BN_enable=True)
        #self.decode2 = DecoderBlock(in_channels=filters[3], mid_channels=filters[2]*4, out_channels=filters[2], BN_enable=True)
        self.decode3 = DecoderBlock(in_channels=filters[1]*2, mid_channels=filters[1]*4, out_channels=filters[1], BN_enable=True)
        self.decode4 = DecoderBlock(in_channels=filters[1], mid_channels=filters[0]*4, out_channels=filters[0], BN_enable=True)
        self.final_cat = nn.Sequential(
    ...
    def cat_features(self,x):
        x_out = self.concate_conv(x)
        x_out = self.concate_bn(x_out)
        x_out = self.concate_relu(x_out)
        return x_out

    def cat_features_2(self,x):
        #print(x.shape)
        x_1 = self.firstconv_1(x[:,0,...].unsqueeze(1))
        x_1 = self.firstbn_1(x_1)
        x_1 = self.firstrelu_1(x_1)
        x_1 = self.firstmaxpool_1(x_1)
        x_1 = self.encoder1_1(x_1)
        #x_1 = self.encoder2_1(x_1)
        #x_1 = self.encoder3_1(x_1)

        x_2 = self.firstconv_2(x[:,1,...].unsqueeze(1))
        x_2 = self.firstbn_2(x_2)
